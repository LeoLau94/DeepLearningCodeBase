...
LOSS_RATIO = 0.9
PRUNE_RATIO = 0.7
STUDENT_PARAMS = '/data/leolau/checkpoints/CIFAR100/ResNet164_preActivation(Pruned)/Sep29_07-13-34/pruned_model.pkl'
TEACHER_PARAMS = '/data/leolau/checkpoints/CIFAR100/ResNet164_preActivation/Sep20_05-00-20/best_performance_model_params.pkl'


# initialize models
loadPKL = torch.load(STUDENT_PARAMS)
model = preactivation_resnet164(cfgs=loadPKL['cfg'], num_classes=NUM_CLASSES)
model.load_state_dict(loadPKL['model_state_dict'])
teacher_model = preactivation_resnet164(num_classes=NUM_CLASSES)
teacher_model.load_state_dict(torch.load(TEACHER_PARAMS))
teacher_model.cuda()
teacher_model.eval()
