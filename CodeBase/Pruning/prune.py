...
__all__ = ['preprocessVGG16', 'transferVGG16']
...
def preprocessVGG16(pruner, ratio):
    ...
    for m in original_model.conv_layers:
        if isinstance(m, nn.BatchNorm2d):
            ...
            normalization = normalize(weight_copy.data.abs())
...
def transferVGG16(pruner, cfg, cfg_mask):
    # transfer the reserved gamma to a newly built model
    original_model = pruner.original_model
    pruned_model= pruner.pruned_model = vgg_diy(
